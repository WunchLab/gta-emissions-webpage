...
legend.update = function(dataArray) {
  ...
  for (var i = 0; i < 8; i++) {
      grades.push(round((Math.min.apply(Math, dataArray) + (gradeInterval * i)), 3));
    }
  // Add title to the legend
  div.innerHTML += '<b>Pressure (hPa)</b><br>'
...
// co2dse,cod,codse,gasp,gaspse,t,tse,amb,ambse,rd1,rd1se,rd2,rd2se,avg time;        
        ...
        if (isNaN(pressure) === false) {
          dataArray.push((Math.round(pressure*1000))/1000);
        }
        ...
        var circleMarker = new L.circleMarker([latitude, longitude], {                     //  We Create a marker positioned and colored corresponding to the data passed
          color: getColor(pressure, dataArray),                                                            // from datasource.txt.
          radius: 9,
          opacity: 0.9,
          fillOpacity: 0.9
          }).bindPopup(
